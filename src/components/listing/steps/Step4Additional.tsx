  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6">
      ...
      <div className="space-y-6">
        {/* 총 매매금액 자동합산 */}
        <div>
          <h3 className="font-medium text-gray-900 mb-3">총 매매금액</h3>
          <div className="px-4 py-3 bg-gray-100 rounded-lg">
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">보증금 + 권리금</span>
              <span className="text-base font-semibold text-gray-900">
                {investmentTotal.toLocaleString()}만원
              </span>
            </div>
            <p className="text-xs text-gray-400 mt-1">Step 2에서 입력한 금액이 자동으로 합산됩니다</p>
          </div>
        </div>

        {/* 매출 */}
        <div>
          <h3 className="font-medium text-gray-900 mb-3">월 매출</h3>
            ...
            <div className="grid grid-cols-2 gap-3 mt-2">
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-3 min-h-[44px] pr-8 border border-gray-300 bg-white text-gray-900 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none"
                />
                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 pointer-events-none">명</span>
              </div>
              <div className="relative">
        ...
        {/* 지출 + 매출대비% */}
        <div>
          <h3 className="font-medium text-gray-900 mb-3">월 지출</h3>
                ...
                <div key={field}>
                  <label className="flex items-center justify-between text-xs text-gray-500 mb-1">
                    <span className="flex items-center gap-1">
                      {label}
                      {isAuto && <span className="text-green-500 font-medium">자동</span>}
                    </span>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className={`w-full px-3 py-3 min-h-[44px] pr-12 border ${isAuto ? "border-green-200 bg-green-50/50" : "border-gray-300 bg-white"} text-gray-900 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none text-right`}
                    />
                    <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 pointer-events-none">만원</span>
                  </div>
                </div>
        ...
        {/* 순이익 자동계산 */}
        <div>
          <h3 className="font-medium text-gray-900 mb-3">월 순이익</h3>
          {autoProfit !== null && (
            <div className="mb-2 px-4 py-2.5 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex justify-between items-center">
                <span className="text-sm text-green-700">자동계산</span>
                <div className="flex items-center gap-2">
                  <span className={`text-base font-semibold ${autoProfit >= 0 ? "text-green-700" : "text-red-500"}`}>
                    {autoProfit.toLocaleString()}만원
                  </span>
                  {data.monthlyRevenue && data.monthlyRevenue > 0 && (
                    <span className={`text-xs font-medium ${autoProfit >= 0 ? "text-green-500" : "text-red-400"}`}>
                      ({((autoProfit / data.monthlyRevenue) * 100).toFixed(1)}%)
                    </span>
                  )}
                </div>
              </div>
              <p className="text-xs text-green-500 mt-1">매출 - 지출 합계 (아래에서 직접 수정도 가능)</p>
            </div>
          )}
          <div className="relative">
            <input
          ...
          <textarea
            ...
            rows={2}
            className="mt-2 w-full px-3 py-2.5 border border-gray-300 bg-white text-gray-900 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 outline-none resize-none"
          />
        </div>
      </div>

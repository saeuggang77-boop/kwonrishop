...
export async function POST(request: NextRequest) {
  try {
    ...
    const categoryWeightsConfig = (config?.categoryWeights as Record<string, number>) || {
      STARTUP: 25,
      PROPERTY: 25,
      FRANCHISE_TALK: 20,
      TIPS: 20,
    ...
    for (const personality of personalities) {
      ...
      const prompt = getPostGenerationPrompt(personality, toGenerate, selectedKeywords, categoryAssignments);

    ...
    <div className="max-w-3xl mx-auto px-4 py-6 pb-24 md:pb-6">
      ...
      <Section title="기본 정보">
        <InfoGrid>
          ...
          {listing.currentFloor && (
            <InfoItem
              label="층수"
              value={`${listing.isBasement ? "지하 " : ""}${listing.currentFloor}층 / ${listing.totalFloor}층`}
            />
          )}
          <InfoItem
            label="주차"
            value={listing.parkingNone ? "불가" : listing.parkingTotal ? `${listing.parkingTotal}대` : "-"}
          />
        ...
        <Section title="수익 정보">
          <InfoGrid>
            ...
            <InfoItem
              label="운영"
              value={
                listing.operationType === "SOLO"
                  ? "혼자"
                  : listing.operationType === "FAMILY"
                    ? "가족/동업"
                    : "직원 고용"
              }
            />
          ...
          <div className="bg-blue-50 rounded-xl p-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <button
                onClick={handleBump}
                disabled={bumping}
                className="flex-1 py-2.5 px-4 bg-orange-500 text-white rounded-lg font-medium hover:bg-orange-600 active:bg-orange-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm"
              >
                {bumping ? "처리 중..." : "🔥 끌어올리기"}
              </button>
              <Link
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-3 z-10 md:static md:border-0 md:p-0 md:mt-6">
        <div className="max-w-3xl mx-auto flex items-center gap-2 md:gap-3">
          ...
          />
          <Link
            href={session ? `/chat?listingId=${listing.id}` : "/login"}

...
export default function HomePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [latestRes, popularRes, franchiseRes] = await Promise.all([
          fetch("/api/listings?limit=8&sort=latest"),
          fetch("/api/listings?limit=4&sort=popular"),
          fetch("/api/franchise?limit=4"),
        ]);

        const [latestData, popularData, franchiseData] = await Promise.all([
          latestRes.json(),
          popularRes.json(),
          franchiseRes.json(),
        ]);
    ...
    <>
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <section className="py-12 border-t border-gray-100">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-gray-900">인기 매물 (조회수 높은)</h2>
            <Link href="/listings?sort=popular" className="text-blue-600 hover:text-blue-700 font-medium">
              더보기 →
            </Link>
          </div>
          {loading ? (
        ...
        <section className="py-12 border-t border-gray-100">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-2xl font-bold text-gray-900">추천 프랜차이즈</h2>
            <Link href="/franchise" className="text-blue-600 hover:text-blue-700 font-medium">
              더보기 →
            </Link>
          </div>
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {franchiseBrands.map((brand) => (
                ...
                >
                  ...
                  {brand.totalStores && (
                    <p className="text-sm text-gray-600">매장 {brand.totalStores.toLocaleString()}개</p>
                  )}
                </Link>
              ))}
            </div>
        ...
        <section className="py-12 border-t border-gray-100">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              ...
              <h3 className="font-bold text-gray-900 mb-2">1. 매물 검색</h3>
            ...
            <div className="text-center">
              ...
              <h3 className="font-bold text-gray-900 mb-2">2. 상세 확인 & 채팅</h3>
              <p className="text-sm text-gray-500">
                매물 상세 정보를 확인하고<br />
                채팅으로 직접 문의하세요
              </p>
            ...
            <div className="text-center">
              <div className="w-16 h-16 mx-auto mb-4 bg-purple-100 rounded-full flex items-center justify-center text-purple-600 text-2xl">
                ✓
              </div>
              <h3 className="font-bold text-gray-900 mb-2">3. 직거래 완료</h3>
              <p className="text-sm text-gray-500">
        ...
        <section className="py-12 border-t border-gray-100">
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="w-12 h-12 mx-auto mb-3 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 text-xl">
                0
              </div>
              <h3 className="font-bold text-gray-900 mb-1">중개수수료 0원</h3>
              <p className="text-sm text-gray-500">
                매도자와 매수자가 직접 거래하여 불필요한 수수료를 절약합니다
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 mx-auto mb-3 bg-green-100 rounded-full flex items-center justify-center text-green-600 text-xl">
                ✓
              </div>
              <h3 className="font-bold text-gray-900 mb-1">사업자 인증</h3>
              <p className="text-sm text-gray-500">
                국세청 API로 인증된 실제 사업자만 매물을 등록할 수 있습니다
              </p>
            </div>
            <div className="text-center">
              <div className="w-12 h-12 mx-auto mb-3 bg-purple-100 rounded-full flex items-center justify-center text-purple-600 text-xl">
                ★
              </div>
              <h3 className="font-bold text-gray-900 mb-1">블라인드 리뷰</h3>
              <p className="text-sm text-gray-500">

  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        /* ===== 브라우즈 모드 (2계층: 추천 + 전체) ===== */
        <>
          {/* 추천 집기 (VIP + PREMIUM 통합) */}
          {(vipEquipments.length > 0 || premiumEquipments.length > 0) && (
            <section className="mb-16">
              <div className="mb-8">
                ...
                <h2 className="text-2xl md:text-3xl font-extrabold text-green-700 tracking-tight">
                  추천 <span className="font-light text-terra-500">집기</span>
                </h2>
              </div>
          ...
          {/* 전체 집기 (BASIC + FREE 통합) */}
          {(basicEquipments.length > 0 || freeEquipments.length > 0) && (
            <section>
                ...
                <div className="w-32 sm:w-44 h-36 sm:h-40 shrink-0 bg-green-800 relative">
                  {eq.images.length > 0 ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <span className="font-extrabold text-5xl text-terra-300">{eq.title.charAt(0)}</span>
                    </div>
                  )}
              ...
              <div className="flex">
                <div className="w-28 h-28 shrink-0 bg-cream-elev relative">
                  {eq.images.length > 0 ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <span className="font-extrabold text-3xl text-green-700">{eq.title.charAt(0)}</span>
                    </div>
                  )}
                </div>
          ...
          >
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs font-serif italic text-green-700 font-medium">
                  {eq.price === 0 ? "무료 나눔" : `${eq.price.toLocaleString()}원`}
                </span>
              </div>
        ...
        {equipments.map((eq) => (
          ...
          >
            <div className="w-7 h-7 rounded-full bg-cream-elev flex items-center justify-center shrink-0 mr-3">
              <span className="text-xs font-light text-green-700">{eq.title.charAt(0)}</span>
            </div>

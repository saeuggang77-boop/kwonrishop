...
export default function FavoritesPage() {
  ...
  if (loading) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-6">
        <h1 className="text-2xl font-extrabold text-green-700 tracking-tight mb-6">관심매물</h1>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="bg-cream-elev rounded-2xl h-32 animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-6 bg-cream-elev rounded-full p-1 max-w-xs">
        <button
          onClick={() => setActiveTab("listing")}
          className={`flex-1 px-4 py-2 rounded-full text-sm font-semibold transition-colors ${
            activeTab === "listing"
              ? "bg-green-700 text-cream shadow-[0_2px_8px_rgba(31,63,46,0.18)]"
              : "text-muted hover:text-ink"
          }`}
        >
          매물 <span className={activeTab === "listing" ? "text-terra-300" : "text-muted"}>{listings.length}</span>
        </button>
        <button
        ...
        >
          집기 <span className={activeTab === "equipment" ? "text-terra-300" : "text-muted"}>{equipLoaded ? equipment.length : "·"}</span>
        </button>
      </div>
      ...
      {activeTab === "equipment" && (
        equipLoading ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="bg-cream-elev rounded-2xl aspect-[3/4] animate-pulse" />
            ))}
          </div>
        ) : equipment.length === 0 ? (
          <div className="text-center py-20 text-muted">
